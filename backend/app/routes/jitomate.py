...
def create_reporte_jitomate(data: ReporteJitomateCreate, user_id: str = Depends(get_current_user_id)):
    ...
    for precio_item in data.precios:
        ...
        if not precio_item.sin_dato:
            if precio_item.precio is None or precio_item.precio < 0:
                raise HTTPException(400, f"Precio inválido para calidad {precio_item.calidad}")
            precios_validos += 1
        ...
        for precio_item in data.precios:
            ...
            cur.execute(
                """INSERT INTO precios_jitomate_calidad
                       (reporte_jitomate_id, calidad, precio, sin_dato)
                   VALUES (%s, %s, %s, %s)""",
                (reporte_id, precio_item.calidad, precio_val, precio_item.sin_dato),
            )
...
def get_reporte_jitomate(reporte_id: int, user_id: str = Depends(get_current_user_id)):
    with get_db() as conn:
        ...
        cur.execute(
            "SELECT calidad, precio, sin_dato FROM precios_jitomate_calidad WHERE reporte_jitomate_id = %s ORDER BY calidad",
            (reporte_id,),
        )
